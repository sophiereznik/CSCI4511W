function GameManager(e, t, i, n) {
    this.size = e, this.inputManager = new t, this.storageManager = new n, this.actuator = new i, this.startTiles = 2, this.inputManager.on("restart", this.restart.bind(this)), this.inputManager.on("keepPlaying", this.keepPlaying.bind(this)), this.setup()
}
...
function Tile(e, t) {
    ...
    function e() {
        setTimeout(function () {
            var i = 0;
            if (i = gameManager.grid.availableCells().length < 5 ? 6 : 4,
                gameManager.move(gameManager.getBestMove(gameManager.grid, i)) === !1)
                for (var n = 0; 4 > n && gameManager.move(n) !== !0; n++);
            e();
        }, 50);
    }
...
var BOARD = 1,
    PLAYER = 0;
GameManager.prototype.getBestMove = function (e, t) {
    for (var i = Number.MIN_VALUE, n = 0, r = 0; 4 > r; r++) {
        var o = e.clone();
        if (o.move(r) !== !1) {
            var a = this.expectimax(o, t - 1, BOARD);
            a > i && (n = r, i = a)
        }
    }
    return n
}, GameManager.prototype.expectimax = function (e, t, i) {
    var n = this;
    if (0 == t) return e.getScore();
    if (i === PLAYER) {
        for (var r = Number.MIN_VALUE, o = 0; 4 > o; o++) {
            var a = e.clone(),
                s = a.move(o);
            if (s !== !1) {
                var l = this.expectimax(a, t - 1, BOARD);
                l > r && (r = l)
            }
        }
        return r
    ...
    if (i === BOARD) {
        for (var r = 0, c = e.availableCells(), u = c.length, o = 0; u > o; o++) {
            var a = e.clone();
            a.insertTile(new Tile(c[o], 4));
            var l = n.expectimax(a, t - 1, PLAYER);
            r += l === Number.MIN_VALUE ? 0 : .1 * l, a = e.clone(), a.insertTile(new Tile(c[o], 2)), l = n.expectimax(a, t - 1, PLAYER), r += l === Number.MIN_VALUE ? 0 : .9 * l
        }
        return r /= u
    }
    ...
    function () {
        ...
        if (!("undefined" == typeof window.Element || "classList" in document.documentElement)) {
            ...
            }, window.DOMTokenList = e, t(HTMLElement.prototype, "classList", function () {
                return new e(this)
            })
        }
        ...
        if (!this.isGameTerminated()) {
            var i, n, r = this.getVector(e),
                o = this.buildTraversals(r),
                a = !1;
            return this.prepareTiles(), o.x.forEach(function (e) {
                o.y.forEach(function (o) {
                    if (i = {
                        x: e,
                        y: o
                    }, n = t.grid.cellContent(i)) {
                        var s = t.findFarthestPosition(i, r),
                            l = t.grid.cellContent(s.next);
                        if (l && l.value === n.value && !l.mergedFrom) {
                            var c = new Tile(s.next, 2 * n.value);
                            c.mergedFrom = [n, l], t.grid.insertTile(c), t.grid.removeTile(n), n.updatePosition(s.next), t.score += c.value, 2048 === c.value && (t.won = !0)
                        } else t.moveTile(n, s.farthest);
                        t.positionsEqual(i, n) || (a = !0)
                    }
                })
            }), a && (this.addRandomTile(), this.movesAvailable() || (this.over = !0), this.actuate()), a
        }
        ...
        for (var e, t = this, i = 0; i < this.size; i++)
                ...
                if (e = this.grid.cellContent({
                    ...
                    for (var r = 0; 4 > r; r++) {
                        var o = t.getVector(r),
                            ...
                            s = t.grid.cellContent(a);
                        if (s && s.value === e.value) return !0
                    }
        return !1
        ...
        for (var e = [], t = 0; t < this.size; t++)
            for (var i = e[t] = [], n = 0; n < this.size; n++) i.push(null);
        return e
    }, Grid.prototype.fromState = function (e) {
        for (var t = [], i = 0; i < this.size; i++)
            for (var n = t[i] = [], r = 0; r < this.size; r++) {
                var o = e[i][r];
                n.push(o ? new Tile(o.position, o.value) : null)
            }
        return t
    }, Grid.prototype.randomAvailableCell = function () {
        var e = this.availableCells();
        return e.length ? e[Math.floor(Math.random() * e.length)] : void 0
        ...
        for (var e = [], t = 0; t < this.size; t++)
            for (var i = e[t] = [], n = 0; n < this.size; n++) i.push(this.cells[t][n] ? this.cells[t][n].serialize() : null);
        return {
            size: this.size,
            cells: e
        }
    }, Grid.prototype.countTileSum = function () {
        for (var e = this, t = 0, i = 0, n = 0; n < e.size; n++)
            for (var r = 0; r < e.size; r++) null != e.cells[n][r] && (i += e.cells[n][r].value, t += 1);
        return i / t
        ...
        for (var e = this, t = new Grid(e.size, null), i = 0; i < e.size; i++)
            for (var n = 0; n < e.size; n++) t.cells[i][n] = null === e.cells[i][n] ? null : new Tile({
                x: e.cells[i][n].x,
                y: e.cells[i][n].y
            }, e.cells[i][n].value);
        return t
        ...
        for (var e = this, t = "", i = 0; i < e.size; i++) {
            for (var n = 0; n < e.size; n++) t += null === e.cells[n][i] ? "0 " : e.cells[n][i].value + " ";
            t += "\n"
        }
        console.log(t)
    }, move = ["up", "right", "down", "left"], Grid.prototype.findFarthestPosition = function (e, t) {
        var i, n = this;
        do i = e, e = {
            x: i.x + t.x,
            y: i.y + t.y
        }; while (n.withinBounds(e) && n.cellAvailable(e));
        return {
            farthest: i,
            next: e
        }
    }, Grid.prototype.moveTile = function (e, t) {
        this.cells[e.x][e.y] = null, this.cells[t.x][t.y] = e, e.updatePosition(t)
        ...
        var t, i, n = this,
            ...
            o = this.buildTraversals(r),
            a = !1;
        return n.eachCell(function (e, t, i) {
            i && (i.mergedFrom = null, i.savePosition())
            ...
            o.y.forEach(function (o) {
                if (t = {
                    ...
                    var s = n.findFarthestPosition(t, r),
                        l = n.cellContent(s.next);
                    if (l && l.value === i.value && !l.mergedFrom) {
                        var c = new Tile(s.next, 2 * i.value);
                        c.mergedFrom = [i, l], n.insertTile(c), n.removeTile(i), i.updatePosition(s.next)
                    } else n.moveTile(i, s.farthest);
                    n.positionsEqual(t, i) || (a = !0)
                }
            })
        }), a
    };
...
Grid.prototype.getScore = function () {
    ...
    this.eachCell(function (e, i, n) {
        n && (t += priority[e][i] * n.value * n.value)
    });
    var i = 0,
        n = [
            [1, 0],
    ...
    return this.eachCell(function (t, r, o) {
        if (o)
            for (var a = 0; 4 > a; a++) {
                var s = {
                    x: t + n[a][0],
                ...
                if (e.withinBounds(s)) {
                    var l = e.cells[s.x][s.y];
                    l && (i += 1 * Math.abs(l.value - o.value))
                }
            }
    }), t - i
    ...
    var t = this,
        ...
        a = ["tile", "tile-" + e.value, o];
    e.value > 2048 && a.push("tile-super"), this.applyClasses(i, a), n.classList.add("tile-inner"), n.textContent = e.value, e.previousPosition ? window.requestAnimationFrame(function () {
        a[2] = t.positionClass({
            x: e.x,
            y: e.y
        }), t.applyClasses(i, a)
    }) : e.mergedFrom ? (a.push("tile-merged"), this.applyClasses(i, a), e.mergedFrom.forEach(function (e) {
        t.addTile(e)
    ...
    return e = this.normalizePosition(e), "tile-position-" + e.x + "-" + e.y
    ...
    document.addEventListener("keydown", function (i) {
        var n = i.altKey || i.ctrlKey || i.metaKey || i.shiftKey,
            r = t[i.which];
        n || void 0 !== r && (i.preventDefault(), e.emit("move", r)), n || 82 !== i.which || e.restart.call(e, i)
    ...
    var i, n, r = document.getElementsByClassName("game-container")[0];
    r.addEventListener(this.eventTouchstart, function (e) {
        !window.navigator.msPointerEnabled && e.touches.length > 1 || e.targetTouches > 1 || (window.navigator.msPointerEnabled ? (i = e.pageX, n = e.pageY) : (i = e.touches[0].clientX, n = e.touches[0].clientY), e.preventDefault())
    }), r.addEventListener(this.eventTouchmove, function (e) {
        e.preventDefault()
    }), r.addEventListener(this.eventTouchend, function (t) {
        if (!(!window.navigator.msPointerEnabled && t.touches.length > 0 || t.targetTouches > 0)) {
            var r, o;
            window.navigator.msPointerEnabled ? (r = t.pageX, o = t.pageY) : (r = t.changedTouches[0].clientX, o = t.changedTouches[0].clientY);
            var a = r - i,
                s = Math.abs(a),
                l = o - n,
                c = Math.abs(l);
            Math.max(s, c) > 10 && e.emit("move", s > c ? a > 0 ? 1 : 3 : l > 0 ? 2 : 0)
        }
    })
